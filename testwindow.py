...
def show_example_menu_file():
    ...
    imgui.menu_item(label="(dummy menu)",
                    shortcut=None,
                    selected=False,
                    enabled=False)
...
def show_test_window():
    ...
    if show_app_style_editor:
        show_app_style_editor = imgui.begin(label="Style Editor",
                                            closable=show_app_style_editor)
    ...
    if imgui.begin_menu_bar():
        ...
        if imgui.begin_menu(label="Examples"):
            ...
            clicked, show_app_log = imgui.menu_item(label="Log",
                                                    shortcut=None,
                                                    selected=show_app_log)
        ...
        if imgui.begin_menu(label="Help"):
            ...
            clicked, show_app_about = imgui.menu_item(label="About Dear ImGui",
                                                      shortcut=None,
    ...
    if show:
        ...
        if imgui.tree_node("Configuration##2"):
            ...
            clicked, io.config_flags = imgui.checkbox_flags(
                "io.ConfigFlags: NoMouse", io.config_flags,
            ...
            imgui.checkbox(label="io.ConfigInputTextCursorBlink",
        ...
        if show:
            clicked, no_titlebar = imgui.checkbox(label="No titlebar",
                                                  state=no_titlebar)
            imgui.same_line(150)
            clicked, no_scrollbar = imgui.checkbox(label="No scrollbar",
                                                   state=no_scrollbar)
            imgui.same_line(300)
            clicked, no_menu = imgui.checkbox(label="No menu", state=no_menu)
            clicked, no_move = imgui.checkbox(label="No move", state=no_move)
            imgui.same_line(150)
            clicked, no_resize = imgui.checkbox(label="No resize",
                                                state=no_resize)
            imgui.same_line(300)
            clicked, no_collapse = imgui.checkbox(label="No collapse",
                                                  state=no_collapse)
            clicked, no_close = imgui.checkbox(label="No close",
                                               state=no_close)
            imgui.same_line(150)
    ...
    if show:
        if imgui.tree_node("Basic"):
            ...
            for i in range(7):
                ...
                imgui.push_style_color(imgui.COLOR_BUTTON, i / 7.0, 0.6, 0.6,
                                       1.0)
                imgui.push_style_color(imgui.COLOR_BUTTON_HOVERED, i / 7.0,
                                       0.7, 0.7, 1.0)
                imgui.push_style_color(imgui.COLOR_BUTTON_ACTIVE, i / 7.0, 0.8,
                                       0.8, 1.0)
            ...
            changed, widgets_basic_i1 = imgui.drag_int("drag int",
                                                       widgets_basic_i1, 1)
            imgui.same_line()
            show_help_marker(
            ...
            changed, widgets_basic_i2 = imgui.drag_int("drag int 0..100",
                                                       widgets_basic_i2, 1, 0,
                                                       100, "%d%%")

            changed, widgets_basic_f1 = imgui.drag_float(
                "drag float", widgets_basic_f1, 0.005)
            changed, widgets_basic_f2 = imgui.drag_float(
                "drag small float", widgets_basic_f2, 0.0001, 0.0, 0.0,
                "%.06f ns")
        ...
        if imgui.tree_node("Bullets"):
            ...
            imgui.bullet_text("Bullet point 2" + os.linesep +
                              "newOn multiple lines")
            imgui.bullet()
        ...
        if imgui.tree_node("Images"):
            ...
            if imgui.is_item_hovered():
                imgui.begin_tooltip()
                region_sz = 32.0
                region_x = imgui.get_mouse_position(
                ).x - pos.x - region_sz * 0.5
                if region_x < 0.0:
                    region_x = 0.0
                elif region_x > my_tex_w - region_sz:
                    region_x = my_tex_w - region_sz
                region_y = imgui.get_mouse_position(
                ).y - pos.y - region_sz * 0.5
                if region_y < 0.0:
                    region_y = 0.0
                ...
                imgui.text("Max: (" + str(region_x + region_sz) + ", " +
        ...
        if imgui.tree_node("Selectables"):
                ...
                if clicked:
                    if imgui.is_mouse_double_clicked(0):
                        selectables_basic_selection[
                            4] = not selectables_basic_selection[4]
                imgui.tree_pop()
            ...
            if imgui.tree_node("Selection State: Multiple Selection"):
                ...
                for n in range(5):
                    ...
                    buf = "Object " + str(n)
                    clicked, selectables_basic_selection_2[
                        n] = imgui.selectable(
                            label=buf,
                            selected=selectables_basic_selection_2[n])
                    if clicked:
            ...
            if imgui.tree_node("Rendering more text into the same line"):
                ...
                clicked, selectables_basic_selected_2[1] = imgui.selectable(
                    label="Hello.cpp",
                    selected=selectables_basic_selected_2[1])
                imgui.same_line(300)
            ...
            if imgui.tree_node("In columns"):
                ...
                for index in range(len(selectables_basic_selected_3)):
                    ...
                    clicked, selectables_basic_selected_3[
                        index] = imgui.selectable(
                            label=label,
            ...
            if imgui.tree_node("Grid"):
                ...
                for index in range(len(selectables_basic_selected_4)):
                    ...
                    clicked, selectables_basic_selected_4[
                        index] = imgui.selectable(
                            label="Sailor",
                    ...
                    if clicked:
                        ...
                        if x > 0:
                            selectables_basic_selected_4[
                                index -
                                1] = not selectables_basic_selected_4[index -
                                                                      1]
                        if x < 3:
                            selectables_basic_selected_4[
                                index +
                        ...
                        if y > 0:
                            selectables_basic_selected_4[
                                index -
                                4] = not selectables_basic_selected_4[index -
                                                                      4]
                        if y < 3:
                            selectables_basic_selected_4[
                                index +
        ...
        if imgui.tree_node("Filtered Text Input"):
            ...
            imgui.same_line()
            show_help_marker("Display all characters as '*'." + os.linesep +
                             "Disable clipboard cut and copy." + os.linesep +
                             "Disable logging." + os.linesep)
            imgui.input_text(
        ...
        if imgui.tree_node("Multi-line Text Input"):
            ...
            flags = imgui.INPUT_TEXT_ALLOW_TAB_INPUT | (
                ...
                if multi_line_text_input_read_only else 0)
        ...
        if imgui.tree_node("Plots Widgets"):
            ...
            if plots_widgets_animate:
                plots_progress += plots_progress_dir * 0.4 * imgui.get_io(
                ).delta_time
            ...
            imgui.same_line(position=0.0,
                            spacing=imgui.get_style().item_inner_spacing.x)
            imgui.text("Progress Bar")

            progress_saturated = (0.0 if (plots_progress < 0.0) else (
                1.0 if plots_progress > 1.0 else plots_progress))

            progress_text = str(progress_saturated * 1753) + "/" + "1753"
            imgui.progress_bar(fraction=plots_progress,
                               size=(0.0, 0.0),
                               overlay=progress_text)
            imgui.tree_pop()
        ...
        if imgui.tree_node("Color/Picker Widgets"):
            ...
            checked, color_picker_hdr = imgui.checkbox(label="With HDR",
                                                       state=color_picker_hdr)
            imgui.same_line()
            show_help_marker(
    ...
    if show:
        if imgui.tree_node("Popups"):
            ...
            imgui.same_line()
            imgui.text_unformatted("<None>" if popups_selected_fish ==
        ...
        if imgui.tree_node("Context menus"):
            ...
            imgui.text("Value = " + str(context_menus_value) +
                       " (<-- right-click here)")
